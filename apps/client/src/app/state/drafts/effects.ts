...
import {
  ...
  SaveDraft,
  SaveDraftFailure,
  SaveDraftSuccess, SetSelectedDraft,
  UpdateDraft,
  UpdateDraftFailure,
  UpdateDraftSuccess
} from './actions';
...
export class DraftsEffects {
  ...
  saveDraft$ = createEffect(() => this.actions$.pipe(
    ...
    mergeMap((action) => zip(this.jobProvider.createJob({ job: action.payload.draft.content as any }), of(action))),
    tap(([job, action]) => {
      const payload = action.payload;
      this.imagesFacade.updateImages(payload.files, job?.id, payload.draft?.content?.name);
    }),
    tap(() => this.notifier.success(this.localizationService.translate('DRAFTS.SAVED'))),
    map(([res, action]) => new SaveDraftSuccess({
  ...
  updateDraft$ = createEffect(() => this.actions$.pipe(
    ...
    tap((action) => {
      const payload = action.payload;
      this.imagesFacade.updateImages(payload.files, payload.draft.content.id, payload.draft.content.name);
    }),
    mergeMap((action) => this.jobProvider.editJob({ job: action.payload.draft.content })),
    map(res => {
      this.notifier.success(this.localizationService.translate('DRAFTS.UPDATED'));
  ...
  deleteDraft$ = createEffect(() => this.actions$.pipe(
    ...
    mergeMap((action) => this.jobProvider.markAsCancel(action.payload)
      .pipe(
        tap(() => this.notifier.success(this.localizationService.translate('DRAFTS.REMOVED'))),
  ...
  loadDrafts$ = createEffect(() => this.actions$.pipe(
    ...
    mergeMap((action) => this.jobProvider.getAuctions({
  ...
  setSelectedDraft$ = createEffect(() => this.actions$.pipe(
