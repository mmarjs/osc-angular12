...
export class MybidsComponent implements OnInit, OnDestroy {
  ...
  constructor(private fb: FormBuilder, private bidsFacade: BidsFacade, private localizationService: LocalizationService) {
  }
  ...
  ngOnInit(): void {
    ...
    this.pagedDataOfMyBidsSubscription = this.bidsFacade.pagedDataOfMyBids$.pipe(
  ...
  onChange(controlName: string) {
    ...
    const pageable = {
      ...
      page: this.source.currentPageNo,
      size: this.source.data.length
    };
    this.bidsFacade.loadMyBids({pageable: pageable});
    Object.keys(this.filterForm.controls).filter(element => element !== controlName)
      .map(ctrl => this.filterForm.get(ctrl).setValue(null));
  }
