...
export class DropZoneDirective {
  ...
  // Accepted types
  @Input()
  set accept(types: Array<string> | string) {
    this._accept =
      typeof types === 'string'
        ? types.split(',').map(v => v.trim())
        : Array.isArray(types)
        ? types
        : [];
  }
  ...
  onDrop(event: DragEvent) {
    ...
    if (dataTransfer.items) {
      const files = Array.from(dataTransfer.items)
        ...
        .map(f => f.getAsFile())
        .filter(f => !this._accept.length || this._accept.includes(f.type));
      ...
      this.fileDrop.emit(
        Array.from(files).filter(
          f => !this._accept.length || this._accept.includes(f.type)
        )
      );
    }
  }
