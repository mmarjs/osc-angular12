...
export class Cells {
  ...
  getCellIndexInContainer(cellIndexInDOMTree: number) {
    ...
    const cellLength = this.cellLengthInLightDOMMode;
  ...
  getImage(cellIndex: number) {
    ...
    const imageIndex = this.getImageIndex(cellIndex);
    const file = this.images[imageIndex];
  ...
  getImageIndex(cellIndexInDOMTree: number) {
    ...
    if (this.counter > this.overflowCellsLimit) {
      const cellLimitOverflow = this.counter - this.overflowCellsLimit;
